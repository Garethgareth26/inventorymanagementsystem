...
## Design Revision Note (v1.2)

| Info | Keterangan |
|---|---|
| Trigger | Client-directed visual redesign, based on a client-provided high-fidelity dashboard reference image (uploaded 5 Juli 2026) |
| Scope | **Visual language only.** Sections 11 (UI Components) and 12 (Design System) below are fully revised, and the visual-style language in Section 17 (Wireframe Description) and Section 18 (Google Stitch Prompt) is updated accordingly. Every screen, field, validation rule, flow, RBAC rule, and data requirement anywhere in this document — Sections 0–10, 13–16, 19, and the Addendum (20–23) — is functionally unchanged. This is a re-skin, not a re-scope. |
| Supersedes | Section 12's v1.0 palette/typography/elevation/animation spec, and the corresponding style language in Sections 17–18 |
| Basis | Client reference image: warm neutral (greige/cream) background, deep sage-green primary accent, fully-rounded/pill-shaped component language (buttons, inputs, badges, floating capsule sidebar), icon-circle KPI cards with one solid-accent "hero" card per row, flat color-block elevation (near-zero shadow), and organic soft-curve data visualization (smooth line charts, thick rounded donut/gauge arcs) |
...
| Component | Notes |
|---|---|
| Buttons | Primary — fully rounded/pill-shaped, solid deep-sage-green fill, white text, optional leading icon (one per view max for the "main" action); Secondary — pill-shaped outline in the same sage tone; Destructive — pill-shaped, solid deep-red fill (delete-class actions only); Disabled — same pill shape, muted/desaturated fill (Owner's blocked write actions — always paired with a tooltip explaining why, never a silently-inert button) |
| Cards | KPI/Stat Card — white rounded-2xl card (~20–24px radius) with a small colored icon-circle (duotone: light tint background + darker icon), label, large bold number, optional trend pill; one KPI per dashboard row renders as a solid-accent "hero" variant (dark sage fill, white text, small inline sparkline) to draw the eye to that row's single most important figure (Section 17.2); Content Card — same white rounded-2xl treatment, generous internal padding, wraps forms/panels |
| Data Tables | Per Section 8 — rows sit on the same white rounded-card surface as every other card, never directly on the page's greige background |
| Charts | Donut (ABC) and radial Gauge — thick rounded stroke, sage-green arc against a light neutral track, value centered in bold; Horizontal Bar (top-5 cost) — rounded bar caps, sage-green fill; Line (usage trend, optional future addition) — a single smooth organic spline, no gridlines — all via ApexCharts (SAD ADR-006), themed to this palette rather than left at ApexCharts' defaults |
| Badges | Pill-shaped only (fully rounded), tinted background + matching darker text — never text-only or square; positive/increase = pale-sage bg + deep-sage text, negative/decrease = pale-rose bg + deep-rose text; PO/ABC/critical-stock status badges keep their existing color-plus-label pairing (Section 14) inside this same pill shape |
| Alerts | Inline banners for form-level and page-level warnings/errors — same white/rounded card family, left accent stripe in the relevant semantic color rather than a fully-filled colored banner, to stay consistent with the flat/color-block elevation model (Section 12) |
| Modals | Per Section 9 — rounded-2xl corners matching the card radius; one of the few places a soft ambient shadow is used (Section 12) |
| Tabs | Used within Master Data detail screens where a record has multiple related views (e.g., Raw Material Detail: Info / EOQ-SS-ROP / History tabs) — rendered as a pill-shaped segmented control rather than underlined tabs |
| Forms | Per Section 7 — input fields are fully rounded/pill-shaped, matching the search box and button language |
| Breadcrumbs | Module Group / Module / Record — e.g., "Master Data / Raw Materials / BB01" — small gray text, unaffected by the visual revision |
| Dropdowns | Searchable select for any foreign-key picker (material, supplier, finished good) — plain select only for small fixed enums (unit, status, role); rounded to match input-field radius |
| Pagination | Per Section 8 — pill-shaped page-number controls |
| Search Box | Pill-shaped input with a circular solid-dark icon button anchored at its right end (matches the reference image's header search bar); consistent placement top-left of table toolbar across all modules, and duplicated in the header per Section 17.1 |
| Filters | Per Section 8 — pill-shaped filter controls |
| Status Chips | Removable chips representing active filters — pill-shaped, matches Badges above |
| Timeline | Used on Purchase Order Detail (Menunggu → Dalam Proses → Diterima) — rendered as connected pill nodes joined by a thin sage-green line, rather than a bare progress bar |
| Navigation Rail (Sidebar) | Floating, fully-rounded (capsule) white rail, detached from the viewport edge with visible margin; icon-only (no text labels), tooltips on hover; active item shown as a solid dark circular background behind the icon; circular user-avatar pinned as the bottom-most item |

---

## 12. Design System

**Design Direction (v1.2 — client-revised):** Warm, calm, and rounded — moving from v1.0's flat corporate-ERP brief toward the soft neutral/sage-green, fully-rounded, icon-circle-driven visual language of the client's reference dashboard, while keeping every functional requirement (information density, scan-ability, accessibility) from the original brief intact. This restyles the same information architecture; it does not redesign the architecture itself.

**Color Palette (Light Theme):**

| Role | Color | Usage |
|---|---|---|
| Page background | Warm off-white / greige (~`#EDEAE3`) | The canvas behind every card — cards sit on this, never directly on white |
| Card surface | White / near-white (~`#FFFFFF`–`#FDFCFA`) | All cards, tables, modals, form panels |
| Primary accent (brand/action) | Deep sage green (~`#3E5C48`–`#4C6A56`) | Primary buttons, active nav state, primary chart fill, "hero" accent KPI card, positive-trend badge text |
| Primary accent — light tint | Pale sage (~`#DCE8DF`) | Icon-circle backgrounds, positive-badge backgrounds, progress-track highlight |
| Secondary accent (warm) | Warm tan/gold (icon ~`#C99B5D`, tint ~`#F3E4C9`) | Secondary icon-circle backgrounds (financial/value-related stats), ABC Class B |
| Negative/decrease | Soft rose (text ~`#C9697A`, tint ~`#F7DEE2`) | Negative-trend badges, "decrease" indicators only — a softer register than Danger below, reserved for metric direction, never for destructive actions |
| Danger/Destructive | Deeper red (~`#B3413F`) | Delete-class actions, hard validation errors (insufficient stock, negative-stock block) — deliberately more saturated than the rose above, so "this number went down" is never visually confused with "this action is destructive" |
| Warning | Warm amber (~`#C98A3B`) | Non-blocking warnings (e.g., large-quantity confirmation) |
| Neutral text — primary | Near-black charcoal (~`#2A2A26`) | Headings, primary numbers |
| Neutral text — secondary | Warm gray (~`#8B8880`) | Labels, captions, secondary text |
| Neutral — borders/dividers | Very light warm gray (~`#E7E4DC`) | Card borders (where used), table row dividers |
| ABC class colors | A = deep sage (primary accent) · B = warm tan (secondary accent) · C = neutral warm gray | Reassigned to this palette; the *meaning* is unchanged from v1.0 — A is highest-priority and gets the most visually prominent color, C is lowest and gets the quietest neutral |

**Typography:** A rounded-terminal geometric sans-serif (e.g., **Plus Jakarta Sans**, **General Sans**, or **Switzer** — any reproduces the reference image's warm, friendly-but-professional letterforms; pick one and use it exclusively). Weight variation (regular/medium/semibold/bold) carries the hierarchy — headings and KPI numbers sit at semibold–bold, body/labels at regular–medium. Tabular figures for numeric/tabular data are unchanged from v1.0 and matter even more now that stat pairs (e.g., "Nilai Saat Ini vs. Hasil Simulasi," Section 17.7) sit side-by-side.

**Spacing:** Unchanged 4px base unit (4/8/12/16/24/32/48) — the rhythm doesn't need to change, only the shapes sitting inside it. Default card internal padding increases one step (e.g., 24px → 32px) to match the reference image's airier card interiors.

**Border Radius — the single biggest visual change from v1.0:**
- Cards: large radius, ~20–24px (was "small-to-medium")
- Buttons, inputs, search boxes, badges/pills, filter chips: **fully rounded** (`border-radius: 9999px`) — now the defining shape of the whole system
- Icon containers and avatars: circular
- Sidebar (navigation rail): fully-rounded capsule, floating with margin from the viewport edge rather than flush against it

**Icons:** Thin, single-stroke outline icons for navigation and inline UI — one consistent set, never mixed (unchanged principle from v1.0). New: KPI/metric icons sit inside a colored circular "duotone" badge (light tint background + darker icon of the same or a complementary hue, per the palette table above) rather than appearing bare — the reference image's icon-circle pattern, applied consistently to every stat card system-wide.

**Elevation:** Still flat, still deliberately low-shadow — unchanged principle from v1.0, achieved differently: cards are differentiated from the greige page background by **color contrast alone**, not by shadow. A very soft, barely-perceptible ambient shadow is reserved for the same short list as before — modals, toasts, dropdowns — plus, newly, the floating sidebar capsule (since it visually detaches from the edge and benefits from a hint of lift).

**Animations — expanded and made explicit per the client's smooth-transition request:**

| Interaction | Specification |
|---|---|
| Page/route transitions | 200–250ms fade + 8px upward slide on the incoming content area (Livewire full-page navigations) — never a hard cut, never long enough to feel like a loading delay |
| Sidebar active-state change | The circular active-item background slides/morphs to the newly-active icon over ~200ms ease-in-out, rather than an instant swap |
| Hover (cards, buttons, table rows) | Subtle 150ms ease-out: buttons/pills scale to ~1.02 and deepen their fill slightly; cards fade in a barely-visible shadow (the one place static cards gain any shadow, and only on hover); table rows tint their background |
| KPI number load-in | Numbers count up from 0 to their final value over ~600–800ms ease-out, first render only — never re-triggered on a routine `wire:poll` refresh (Section 15's polling-update rule still applies: a poll updates the number directly, no re-animation, so frequent updates don't become visually noisy) |
| Chart draw-in | Line/spline charts draw left-to-right over ~500ms on first load; donut/gauge arcs sweep from 0 to their value over ~500ms; bars grow from baseline over ~400ms with a slight (~40ms) stagger between bars — first-load only, same no-re-animate-on-poll rule as above |
| Skeleton → content | Cross-fade, ~200ms, rather than an abrupt swap |
| Toast enter/exit | Unchanged from v1.0: brief slide-in from the top-right + fade, auto-dismiss per Section 10 |
| Modal open/close | Same principle as v1.0, updated easing: scale-from-98%-to-100% + fade, ~200ms, matching the new rounded-corner card shape |
| Focus/validation states | Instant-to-150ms — deliberately *not* part of the "smooth and slow" language above, since validation feedback needs to read as immediate, not decorative |

The governing principle is unchanged from v1.0: animation communicates *state change* (loaded, active, hovered, in-error) and never adds perceived latency to a repeated daily action. Only the specific durations/easings above are new — "smooth transitions" was the client's explicit ask and deserved concrete numbers rather than the vaguer "purposeful and brief" language it replaces.

**Light Theme:** Fully specified above (still the v1 deliverable).

**Dark Theme:** Still explicitly future/out of scope, unchanged from v1.0 — the new sage/greige/tan/rose hues were chosen the same way the old blue/gray palette was: avoiding pure black/white and colors that lose meaning when inverted, so a future dark theme still isn't a rebuild.
...
- **Header** (full width, fixed top, ~72–80px, sits on the page's greige background rather than a hard white bar): left — a small brand mark + the module breadcrumb (the Dashboard specifically may show a light "Hello, {name}" greeting in its place, since it has no deeper hierarchy above it, matching the reference image); right — a pill-shaped search input with a circular icon button at its end, a circular notification-bell button (with badge), a circular user avatar/name dropdown (Profile — not in scope, Logout) — each floating as its own white circular/pill control rather than flat header icons.
- **Sidebar** (floating rounded-full capsule rail, detached from the viewport edge with visible margin, ~72px wide, icon-only at every breakpoint): module groups per Section 1; active state shown as a solid dark circular background behind the active icon (replacing the v1.0 left-accent-bar treatment).
- **Content Area** (fills remaining space, scrollable independently of sidebar/header): page title + primary action button in a header row, then the page's specific content below — unchanged structurally from v1.0.
...
Content area: 
- Row 1: KPI cards in a horizontal grid (4 for Owner, 3 for Employee), equal width; each card carries a small colored icon-circle (Section 11/12) + label + big bold number + optional trend pill. The single most business-critical KPI in the row (Owner: Nilai Stok Total; Employee: Jumlah Item Kritis) renders as the solid dark-sage "hero" card variant with a small inline sparkline, matching the reference image's accent-card treatment — a styling choice only, it doesn't change which KPIs are shown (Section 5).
- Row 2 (Owner only) / Quick Actions Bar (Employee only): Owner sees a two-column split — ABC Donut Chart (left, ~40% width, thick rounded sage arc + centered class-share %) and Top-5 Cost Bar Chart (right, ~60% width, rounded bar caps) side by side; Employee sees three large pill-shaped tappable action buttons in a row instead.
- Row 3: Critical Stock Table, full width, sitting on the same white rounded-card surface as every other card on the page, with its own mini-header ("Live Stock Critical Alert" + last-updated timestamp reflecting the polling cadence).
- Row 4: two-column split — Recent Activity Feed (left, ~50%), each entry styled with a thin colored accent bar on its left edge, description + timestamp, and an optional pill badge at the row's right edge — and Upcoming Reorders Panel (Owner) or nothing/secondary content (Employee) (right, ~50%).
...
> Design a modern Inventory Management System UI for "CV Akuna," an internal web application (Laravel + Livewire backend, not needed for the design) used by warehouse staff and business owners of a small cosmetics/personal-care manufacturer. Style: warm, calm, and rounded — a soft sage-green-and-cream palette, fully-rounded pill-shaped buttons/inputs/badges, large-radius white cards floating on a warm greige page background, icon-circle KPI stats, and flat, color-block elevation (near-zero shadow) rather than a hard-edged corporate-blue aesthetic — think a friendly fintech/wellness dashboard's visual warmth applied to a data-dense operational tool, not a playful consumer app. Tailwind CSS visual language (spacing, type scale, component shapes), desktop-first responsive layout (primary target 1280px+, graceful degradation to tablet/mobile). Accessible design: strong color contrast even within the softer palette, visible focus states, never color-only status indicators.
>
> **Layout shell:** a floating, icon-only, fully-rounded (capsule/pill) left navigation rail detached from the viewport edge, with the active module shown as a solid dark circular background behind its icon; a header on the page's warm background (not a hard white bar) containing a pill-shaped search input, a circular notification-bell button, and a user-avatar menu; main content area with a page-title-and-primary-action header row on every screen (the Dashboard alone may swap this for a light "Hello, {name}" greeting, since it has no deeper page hierarchy above it).
>
...
>
> **Design a dashboard for each role:** the Owner dashboard shows 4 KPI cards (each with a small colored icon-circle; the single most important one — Total Stock Value — rendered as a solid sage-green "hero" card with an inline sparkline), an ABC classification donut chart (thick rounded sage arc, centered percentage), a horizontal bar chart of the 5 most expensive raw materials (rounded bar caps), a live "critical stock" alert table (materials below their reorder point) on its own white rounded card, a recent activity feed styled as a list with a thin colored accent bar per row, and an upcoming-reorders panel — with zero create/edit buttons anywhere on the page. The Employee dashboard shows 3 KPI cards in the same icon-circle style, the same critical-stock table but with an inline pill-shaped "Create Emergency Purchase Order" action per row, three large pill-shaped quick-action buttons (New Purchase Order, Record Production, Adjust Stock), and a recent-activity feed of the user's own actions in the same accent-bar list style.
>
> **Design these core modules as data tables with search, filters, sorting, and pagination:** Suppliers; Raw Materials (with an ABC-class pill badge column); Finished Goods; Purchase Orders (with status badges: Menunggu/Dalam Proses/Diterima, and order-type badges: Rutin/Darurat-with-urgent-styling); Production Entries; a unified Inventory Movements ledger (immutable, read-only, filterable by source: manual/purchase-receipt/production); EOQ, Safety Stock, and Reorder Point overview tables; an ABC Analysis report combining a donut chart with a ranked table.
>
> **Design these key forms/interactive screens:** a Bill of Materials editor (repeatable ingredient-line rows: material picker, quantity-per-unit, unit, remove icon); a "Create Production Entry" screen that shows a live BOM-explosion preview table (required quantity vs. available stock vs. resulting stock per raw material, with a clearly-distinct "insufficient" state — using the deeper destructive-red, not the softer negative-metric rose — that disables the submit button when any line is short; this hard validation is a core business rule, make it visually unmissable even within the softer palette); an EOQ/Safety-Stock/Reorder-Point simulation screen with an editable input panel on the left and a side-by-side "current value vs. simulated result" comparison table on the right, plus the formula shown in small print for transparency; a Report Generator with 3 selectable report-type cards (Warehouse Asset Valuation, Supplier Performance, Monthly Movement), a date-range picker, and a generation-progress state.
>
> **Notification system:** a circular bell-icon button with a badge count in the header, opening a rounded dropdown list of currently-critical materials; toast notifications (success=sage-green, error=deep-red, warning=amber) stacking top-right, pill-shaped with rounded corners matching the rest of the system.
>
> **Visual style specifics:** warm greige page background (~`#EDEAE3`) with white/near-white cards; deep sage green (~`#3E5C48`) as the primary action/brand color with a pale sage tint (~`#DCE8DF`) for icon-circle backgrounds and positive-trend badges; warm tan/gold as a secondary accent for financial/value-related icon circles; soft rose for negative/decrease metric badges, kept visually distinct from a deeper, more saturated red reserved only for destructive actions and hard validation errors; amber for non-blocking warnings; a rounded-terminal geometric sans-serif typeface (e.g., Plus Jakarta Sans or General Sans) throughout, tabular figures for numeric columns; large card radius (~20–24px) and fully-rounded/pill shapes for every button, input, search box, and badge — this pill/capsule language, including the floating capsule sidebar, is the single most defining visual trait of this design; flat, color-block elevation with virtually no drop shadow on static cards (differentiation comes from white-card-on-greige-page contrast, not shadow) — the same short list as before (modals, toasts, dropdowns) keeps a soft ambient shadow, plus the floating sidebar; one consistent thin single-stroke outline icon set throughout, with KPI/metric icons additionally sitting inside colored duotone circles; smooth, purposeful transition animations with specific timing (~150–250ms for hovers/micro-interactions, ~400–800ms for chart draw-ins and first-load KPI count-ups, never re-triggered on routine data-poll refreshes).
>
> This is an internal business application — prioritize information density, scan-ability, and fast repeated daily use over decorative visual flourish, even within this warmer, rounder visual language; the softness of the palette and shapes should read as calm and approachable, not as reduced data density. Include: Login screen (centered card, no chrome), the two role-specific dashboards, at least one full data-table module (Raw Materials or Purchase Orders), the Production Entry screen with its BOM explosion preview, and one EOQ/Safety-Stock/Reorder-Point simulation screen with the comparison view.
